...
#        type=int)
    ...
    parser.add_argument(
        '--hw',
        nargs=2,
        help='The height and width of image.'
        'For example, `--hw 512 512`',
        type=int)
...
def main(args):
    """
    For example:
    python3 tools/flops.py --config configs/gc_mobileseg/gc_mobileseg_base_ade20k_512x512_80k.yml --hw 512 512
    """
...
#    logger.info('The number of images: {}'.format(len(image_list)))
    logger.info('The height and width of images: {}'.format(args.hw))

    paddle.flops(model, [1, 3, args.hw[0], args.hw[1]])
